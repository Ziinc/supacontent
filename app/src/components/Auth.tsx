...
const Auth = () => {
  ...
  const forgotPassword = async (e) => {
    ...
    if (email === null || email === "") {
      ...
      let { error } = await client.auth.api.resetPasswordForEmail(email);
      if (error) {
  ...
  return (
    <div
      className={
        "w-full h-full sm:h-auto sm:w-2/5 max-w-sm p-5 bg-white shadow flex flex-col text-base mx-auto my-auto"
      }
    >
      <span
        className={
